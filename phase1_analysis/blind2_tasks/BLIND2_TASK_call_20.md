...
Then read and follow `calls/call_20/CALL_20_TASK.md`.

CRITICAL RULES:

- Do NOT read any files named `CALL_20_GRADING.json`, `CALL_20_BLIND.json`, or `CALL_20_BLIND2.json` if they appear.
...
- Grade the call from scratch using only:
  ...
  - Transcript files under `calls/call_20/` (`transcript-2.vtt` and, if useful, `paragraphs-2.json` / `sentences-2.json` for navigation).
...
- Write your JSON result to `calls/call_20/CALL_20_BLIND2.json`.
...
- In chat, only confirm with a short message like:
  - `DONE call_20 -> calls/call_20/CALL_20_BLIND2.json`.
