...
Then read and follow `calls/call_12/CALL_12_TASK.md`.

CRITICAL RULES:

- Do NOT read any files named `CALL_12_GRADING.json`, `CALL_12_BLIND.json`, or `CALL_12_BLIND2.json` if they appear.
...
- Grade the call from scratch using only:
  ...
  - Transcript files under `calls/call_12/` (`transcript-2.vtt` and, if useful, `paragraphs-2.json` / `sentences-2.json` for navigation).
...
- Write your JSON result to `calls/call_12/CALL_12_BLIND2.json`.
...
- In chat, only confirm with a short message like:
  - `DONE call_12 -> calls/call_12/CALL_12_BLIND2.json`.
