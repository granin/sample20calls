...
Then read and follow `calls/call_11/CALL_11_TASK.md`.

CRITICAL RULES:

- Do NOT read any files named `CALL_11_GRADING.json`, `CALL_11_BLIND.json`, or `CALL_11_BLIND2.json` if they appear.
...
- Grade the call from scratch using only:
  ...
  - Transcript files under `calls/call_11/` (`transcript-2.vtt` and, if useful, `paragraphs-2.json` / `sentences-2.json` for navigation).
...
- Write your JSON result to `calls/call_11/CALL_11_BLIND2.json`.
...
- In chat, only confirm with a short message like:
  - `DONE call_11 -> calls/call_11/CALL_11_BLIND2.json`.
