...
Then read and follow `calls/call_15/CALL_15_TASK.md`.

CRITICAL RULES:

- Do NOT read any files named `CALL_15_GRADING.json`, `CALL_15_BLIND.json`, or `CALL_15_BLIND2.json` if they appear.
...
- Grade the call from scratch using only:
  ...
  - Transcript files under `calls/call_15/` (`transcript-2.vtt` and, if useful, `paragraphs-2.json` / `sentences-2.json` for navigation).
...
- Write your JSON result to `calls/call_15/CALL_15_BLIND2.json`.
...
- In chat, only confirm with a short message like:
  - `DONE call_15 -> calls/call_15/CALL_15_BLIND2.json`.
