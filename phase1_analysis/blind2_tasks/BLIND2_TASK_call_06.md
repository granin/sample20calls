...
Then read and follow `calls/call_06/CALL_06_TASK.md`.

CRITICAL RULES:

- Do NOT read any files named `CALL_06_GRADING.json`, `CALL_06_BLIND.json`, or `CALL_06_BLIND2.json` if they appear.
...
- Grade the call from scratch using only:
  ...
  - Transcript files under `calls/call_06/` (`transcript-3.vtt` and, if useful, `paragraphs-3.json` / `sentences-3.json` for navigation).
...
- Write your JSON result to `calls/call_06/CALL_06_BLIND2.json`.
...
- In chat, only confirm with a short message like:
  - `DONE call_06 -> calls/call_06/CALL_06_BLIND2.json`.
