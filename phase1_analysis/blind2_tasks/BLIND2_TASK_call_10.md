...
Then read and follow `calls/call_10/CALL_10_TASK.md`.

CRITICAL RULES:

- Do NOT read any files named `CALL_10_GRADING.json`, `CALL_10_BLIND.json`, or `CALL_10_BLIND2.json` if they appear.
...
- Grade the call from scratch using only:
  ...
  - Transcript files under `calls/call_10/` (`transcript-2.vtt` and, if useful, `paragraphs-2.json` / `sentences-2.json` for navigation).
...
- Write your JSON result to `calls/call_10/CALL_10_BLIND2.json`.
...
- In chat, only confirm with a short message like:
  - `DONE call_10 -> calls/call_10/CALL_10_BLIND2.json`.
