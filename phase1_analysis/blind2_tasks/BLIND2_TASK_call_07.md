...
Then read and follow `calls/call_07/CALL_07_TASK.md`.

CRITICAL RULES:

- Do NOT read any files named `CALL_07_GRADING.json`, `CALL_07_BLIND.json`, or `CALL_07_BLIND2.json` if they appear.
...
- Grade the call from scratch using only:
  ...
  - Transcript files under `calls/call_07/` (`transcript-2.vtt` and, if useful, `paragraphs-2.json` / `sentences-2.json` for navigation).
...
- Write your JSON result to `calls/call_07/CALL_07_BLIND2.json`.
...
- In chat, only confirm with a short message like:
  - `DONE call_07 -> calls/call_07/CALL_07_BLIND2.json`.
