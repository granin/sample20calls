## TASK: Grade `call_15` and write JSON output

You are a grading agent working **inside this repository**.

Your job in this task is to:
- Grade **`call_15`** using the **Phase 1, 17‑criteria transcript-only system**.
...
Call data:
- `calls/call_15/transcript-2.vtt`
- `calls/call_15/transcript-2.srt`
- `calls/call_15/paragraphs-2.json`
- `calls/call_15/sentences-2.json`
- `calls/call_15/timestamps-2.json`

### What to do

1. Load the instructions and schema from `config/` and `docs/`.
2. Evaluate all 17 Phase 1 criteria for `call_15` using the data in `calls/call_15/`.
3. Produce one JSON object that strictly follows `config/phase1_output_format_v2.json`.
4. **Write** that JSON object to:
   - `calls/call_15/CALL_15_GRADING.json`
