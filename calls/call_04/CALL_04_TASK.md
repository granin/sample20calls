## TASK: Grade `call_04` and write JSON output

You are a grading agent working **inside this repository**.

Your job in this task is to:
- Grade **`call_04`** using the **Phase 1, 17‑criteria transcript-only system**.
...
Call data:
- `calls/call_04/transcript-2.vtt`
- `calls/call_04/transcript-2.srt`
- `calls/call_04/paragraphs-2.json`
- `calls/call_04/sentences-2.json`
- `calls/call_04/timestamps-2.json`

### What to do

1. Load the instructions and schema from `config/` and `docs/`.
2. Evaluate all 17 Phase 1 criteria for `call_04` using the data in `calls/call_04/`.
3. Produce one JSON object that strictly follows `config/phase1_output_format_v2.json`.
4. **Write** that JSON object to:
   - `calls/call_04/CALL_04_GRADING.json`
