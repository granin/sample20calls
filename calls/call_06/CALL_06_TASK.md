## TASK: Grade `call_06` and write JSON output

You are a grading agent working **inside this repository**.

Your job in this task is to:
- Grade **`call_06`** using the **Phase 1, 17‑criteria transcript-only system**.
...
Call data:
- `calls/call_06/transcript-3.vtt`
- `calls/call_06/transcript-3.srt`
- `calls/call_06/paragraphs-3.json`
- `calls/call_06/sentences-3.json`
- `calls/call_06/timestamps-3.json`

### What to do

1. Load the instructions and schema from `config/` and `docs/`.
2. Evaluate all 17 Phase 1 criteria for `call_06` using the data in `calls/call_06/`.
3. Produce one JSON object that strictly follows `config/phase1_output_format_v2.json`.
4. **Write** that JSON object to:
   - `calls/call_06/CALL_06_GRADING.json`
