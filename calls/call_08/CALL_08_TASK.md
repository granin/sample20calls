## TASK: Grade `call_08` and write JSON output

You are a grading agent working **inside this repository**.

Your job in this task is to:
- Grade **`call_08`** using the **Phase 1, 17‑criteria transcript-only system**.
...
- `calls/call_08/transcript-2.srt`
- `calls/call_08/paragraphs-2.json`
- `calls/call_08/sentences-2.json`
- `calls/call_08/timestamps-2.json`

### What to do

1. Load the instructions and schema from `config/` and `docs/`.
2. Evaluate all 17 Phase 1 criteria for `call_08` using the data in `calls/call_08/`.
3. Produce one JSON object that strictly follows `config/phase1_output_format_v2.json`.
4. **Write** that JSON object to:
   - `calls/call_08/CALL_08_GRADING.json`
